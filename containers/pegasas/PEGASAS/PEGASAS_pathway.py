...
def loadGeneSet(fin):
    """Load gene signatures from a GMT-like file (tab-separated)."""
    geneSet = {}
    for line in open(fin):
        ...
        if gene_list_des.find(".") != 0:
            gene_list_des = gene_list_des.rsplit(".", 1)[-1].strip("\t").strip()
...
def loadExpSet(fin):
    """Load gene expression matrix (samples × genes)."""
    geneExp = {}
    header = []
...
def KS_test(l_hits, l_null, l_w, plotting, sample_name):
    """KS two-sample test; returns (enrichment_score, p_value, median_hits_rank)."""
    ...
    for gs in geneSet:
        ...
        for i, sample_name in enumerate(geneExp.keys()):
            ...
            if len(batch_list) == numInterval:
                KS_ES_list = []
                for n in range(0, numInterval):
                    KS_ES_list.append(
                        mp.Process(
                            target=SampleEnrichment,
                            args=(
                                batch_list[n],
                                geneSet[gs],
                                batch_list_s[n],
        ...
        if batch_list:
            KS_ES_list = []
            for n in range(0, len(batch_list)):
                KS_ES_list.append(
                    mp.Process(
                        target=SampleEnrichment,
                        args=(
        ...
        os.system(
            ...
            + score_outdir.rstrip("/")
            + "/*.txt >"
            + score_outdir.rstrip("/").split(".pathway")[0]
            + ".scores.txt"
        )
        print("merged")
